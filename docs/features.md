...
1. **Site nav** — Home and Weeks. Inspiration joins the nav when `INSPIRATION_ENABLED` is on. Hidden if Home is the only item.
...
4. **Inspiration** — notes and ideas (off by default). Toggle `INSPIRATION_ENABLED` in `src/consts.ts`. When on: the homepage lists posts; `/inspiration/` is the full index; each note has its own page; RSS lives at `/inspiration/rss.xml`. When off: no homepage block, and the routes redirect or 404.
5. **Weeks** — `/weeks/` is an 80×52 life calendar. Cells color themselves from today (past, this week, future). Marked weeks can show one emoji and a short factual comment.
